...
Retomando los conceptoa anteriores, consideraremos una **población**, **muestra** y **unidades**, de las últimas podremos establecer **funcione** que traduciremos como **variables** a través de un proceso ergódco. 
...
Una solución al problema anterior estará en lugar de concentrarnos en calcular $T_{i}$ calculamos $T$ que será el **Efecto  medio de tratamientoo** calculado como: $T = E(Y_{i}^{t} - Y_{i}^{c}) = E(T_{i}) $ el cual se obtiene directamente recordando que la media es un operador lineal. 
...
Entonces  $E(\hat{T}) = T$ se cumplirá si $S$ es independiente de $Y$, así el grupo tratado es una muestra aleatoria del grupo de los potencialmente tratados, mismo caso para el grupo de control, así podemos notar que la asignación aleatoria de $S$ hace que la independencia sea plausible pero no necersaiemnte testeable. 

## Problema de identificación. 

Cuando el supuesto de independencia entre $S$ y $Y$ cuando hay alguna relación a priori entre ambos, por ejemplo, si el tratamiento es parte de un procceso de optimización que involucra al resultado, caso que es común.A partir de lo anterior, nos referimos al problema de identificación a la dificultad de obtener un estimador insesgado para identificar parámetros causales a menos que el mecanismo de asignación cumpla con distintas condiciones. 
