...
Cuando hablamos de un proceso de inferencia causal nos referimos al proceso de hacer afirmaciones sobre una **población** basándonos en una **muestra** representativa compuesta de **unidades**, las cuales estarán sujetas al cumplimiento de diferentes supuestos, particularmente, usaremos estos dos conceptos para entender el efecto de un fenómeno ocurrido de la **muestra** sobre la **población**, buscando si es **estadísticamente significativo** para establecer una **relación causal**.

## Modelo de Rubin, Holland 

Una afirmación bastante conocida es que correlación no implica causalidad, sin embargo no resulta muy claro el camino a seguir estadísticamente para crear un modelo causal, una parte importante es establecer el concepto de **aleatoreidad** lo cual obedece a un **proces generador de datos** (data generating process) mismo que dependerá de las condiciones dadas y bucará tener unidades equiprobables.

Retomando los conceptoa anteriores, consideraremos una **población**, **muestra** y **unidades**, y de las últimas podremos establecer **funcione** que traduciremos como **variables** a través de un proceso ergódco. 

Así, podemos definir los componentes del modelo como: 

* Unidad : Será un elemento $i$ en un punto del tiempo específico. 
* Tratamiento : O también llamado intervención $t$, cuyos efectos queremos estimar comparando con un grupo de **control**  $c$ determinado por la ausencia de intervención. 
* Estado del mundo: $S$ para la unidad $i$ al cual nos referiremos cuando exista el tratamiento definido por $S \in {t,c}$. 
* Resultado potencial: Antes de que el tratamiento es asignado, podemos estimar la medición de nuestro resultado de interés $Y$ para cada unidad $i$, bajo ambos grupos de tratamiento y de control, identificados como $Y_{i}^{t}$ y $Y_{i}^{c}$.
* Mecanismo de asignación: El algoritmo que determina si cada $i$ recibe $t$ o $c$.
* Efecto causal: La diferencia entre el resultado potencial y los resultados después del tratamientopara una unidad dada: $Y_{i}^{t} - Y_{i}^{c} = T_{i}$.


Una vez que $S = c$ podemos obtener el **efecto del tratamiento** como $Y_{i}^{t} - Y_{i}^{c} = T_{i}$. No obstante una vez que $S$ está determinado solo podemos observar un único resultado, por lo que nunca observaremos dos resultados potenciales para la misma unidad, a esto se le conocerá como el Problema Fundamental de la Inferencia Causal, es decir, es imposible observar el valor de  $Y_{i}^{t}$ y $Y_{i}^{c}$ para la misma unidad y por tanto, es imposible observar el efecto de $t$ contra $c$ para una $i$ en particular.  

// 

Una solución al problema anterior estará en lugar de concentrarnos en calcular $T_{i}$ calculamos $T$ que será el **Efecto  medio de tratamientoo** dado por: $T = E(Y_{i}^{t} - Y_{i}^{c}) = E(T_{i}) $ el cual se obtiene directamente recordando que la media es un operador lineal. 

Aunque no siempre podemos observar $T$, podemos estimarlo de la siguiente forma: 

$\hat{T} = \hat{E}(Y_{i}^{t}| S=t) - \hat{E}( Y_{i}^{c}|S=c)$

Algo que es importante recordad es que generalmente $\hat{T}$ es un buen estimador de $T$ (no quiere decir que sea el único), lo cual depende del mecanismo de asignación.

Consideremos: $E(Y_{i}^{t}) = (Y_{i}^{t}| S=t)$ y $E(Y_{i}^{c}) = (Y_{i}^{c}| S=c)$

Entonces  $E(\hat{T}) = T$ se cumplirá si $S$ es independiente de $Y$, así el grupo tratado es una muestra aleatoria del grupo de los potencialmente tratados, mismo caso para el grupo de control, por lo que podemos notar que la asignación aleatoria de $S$ hace que la independencia sea plausible pero no necersaiemnte testeable. 

## Problema de identificación. 

Sabremoq que el supuesto de independencia entre $S$ y $Y$ no se cumplirá cuando hay exista alguna relación a priori entre ambos, por ejemplo, si el tratamiento es parte de un procceso de optimización que involucra al resultado, caso que es común.A partir de lo anterior, nos referimos al problema de identificación a la dificultad de obtener un estimador insesgado para identificar parámetros causales a menos que el mecanismo de asignación cumpla con distintas condiciones. 
...
Los vectores se usan regularmente para establecer una secuencia numérica, cada elemento del vector revela información sobre los datos en general y se representan comunmente como: $(S_{1}.... S_{6})$, en este caso se tratará de un vector de 6 elementos. 
...
Un data frame contendrá información sobre múltiples unidades para diferentes vectores, los elementos que contienen este data frame regularmente a nivel de fila tendrán identificadores, aunque no siempre puede darse el caso en el que la información esté indexada, esta última característica será útil si queremos agrupar nuestros datos con algún identificador único de órden.
...
* Muestra aleatoria: se refiere a una realización aleatoria de un conjunto de objetos en un data frame, esperamos que al tomar subconjuntos de dicha muestra:
    ...
    * La media de ese subconjunto converga a la media de la muestra completa.
...
## Distribuciones: nos muetran el comportamiento las varibales aleatorias, y son particularmente útiles para realizar pruebas de hipótesis. Podemos distinguir entre: 
  ...
  * Discretas: Involucran descontinuidades escalonadas en densidad, también tienen la naturaleza de mostrar realizaciones binarias, es por eso que los ejemplos más comunes son las distribuciones Bernoulli y Binomial. 
...
Considerando lo anterior, si queremos estimar una varable instrumental (IV), para el caso de tener una $X$ y una $Z$ usamos la forma analítica dada por  

\begin{equation}
\hat{\beta_{1}^{IV} }= \frac{\hat{Cov(Z,Y)}}{\hat{Cov(Z,X)}} = \frac{\sum_{i=1}^{n}(Z_{i} - \bar{Z})(Y - \bar{Y})}{\sum_{i=1}^{n}(Z_{i} - \bar{Z})(X_{i} - \bar{X})}
\end{equation}

Será conocido como el estimador de Wald, sin embargo, si queremos usar una forma más general para usar para más de una $X$ y más de una $Z$ a la cual nos referiremos como 2SLS, donde nos referiremos como:

1. First stage: Correr la regesión: $X_{1} = \pi_{0} + \pi_{1}Z_{1} + \pi_{2}Z_{2} + u$ y después calculamos  $\hat{X_{1}} = \hat{\pi_{0}} + \hat{\pi_{1}}Z_{1} + \hat{\pi_{2}}Z_{2} + u$ en este paso vamos a intentar predecir la variación en $X$

2. Second stage: Correr la regesión: $Y = \beta_{0} + \beta_{1}\hat{X_{1}} + \epsilon$ Tomamos la variación predecida y la usaremos como un parámetro en la second stage

En caso de que tengamos varias $X$ endógenas, podemos correr el paso 1 para cada $X$, en cada caso se tomará el valor predecido de $X_{1}$ que será una variable exógena plausible si $Z$ es un instrumento válido.

Así 2SLS nos lleva al mismo resultado que nuestra fórmula para IV en el caso donde tenemos una $X$ y una $Z$, tal que obtendremos:

\begin{equation}
\hat{X_{1}} = \pi_{0} + \frac{Cov(X_{1},Z)}{Var(Z)}Z
\end{equation}

E introduciendo el estimador $\hat{X_{1}}$ obtenemos:

\begin{equation}
Y = \beta_{0} + \beta_{1}\hat{X_{1}} + \epsilon = \beta_{0} + \beta_{1}( \pi_{0} + \frac{Cov(X_{1},Z)}{Var(Z)}Z) + \epsilon 
\end{equation}

Y despejando $beta_{1}$

\begin{equation}
\beta_{1} = \frac{Cov(Y, \frac{Cov(X_{1},Z)}{Var(Z)}Z)}{Var (\frac{Cov(X_{1},Z)}{Var(Z)Z})}
= \frac{\frac{Cov(X_{1},Z)}{Var(Z)}Cov(Y,Z)}{\frac{Cov(X_{1},Z)^2}{Var(Z)^2}Var(Z)} = \frac{Cov(Y,Z)}{Cov(X_{1},Z)}
\end{equation}

Que será nuestro estimador de Wald como vimos anteriormente. 

Así, si tenemos una especificación con diferentes regresores endógenos podemos pensar en:

\begin{equation}
Y_{I} = \beta_{0} + \beta_{1}X_{1i} + ... + \beta_{K}X_{ki} + \beta_{k+1}W_{1i} + ... + \beta_{k+r}W_{ri} + u_{i} 
\end{equation}

Donde:

* $X_{1i},...,X_{ki}$ son variables endógenas (posiblemente correlacionds con $u$).
* $W_{1i},...,W_{ri}$ son variables exógenas (no correlacionadas con $u$).
* $Z_{1i},...,Z_{mi}$ son $m$ instrumentos.

Notemos que 2SLS requiere que $m \geq k$ i.e, necesitamos al menos tantos instrumentos como variables endógenas, (no podremos tener menos instrumentos que variales endógenas).

Notemos que $W_{ri}$ denotarán las variables que vamos a incluir en nuestra especificación como controles, mismos que no estrán correlacionados con el término de error $u$

Así el proceso de estimación será:

1. First - stage: Corremos regresión de $X_{1i}$ en todos $Z_{ji}$ y $W_{ji}$, calculamos los valores predecidos de $\hat{X_{1i}}$ y repetimos el proceso para todos los $k$ en las $X_{j}$  variables. 

2. Second - stage: Corremos regresión de $Y_{i}$ sobre todas las predicciones del first-stage, i.e todas las $\hat{X_{ji}}$ y todos los $W_{ji}$, así el estimador $\hat{\beta}$ será el estimados 2SLS de $\beta$. 


Ahora bien, para ilustrar lo anterior, podemos tomar el ejemplo del gusto por tener hijos, es importante tener en mente que no podremos calibrar exactamente cuánta variación en la oferta laboral está dado por el tamaño de la familia ligado al gusto por tener hijos o hijas relativo a alguna variablele omitida como la del gusto de tener hijos o hijas, en la siguiente tabla podemos observar que con MCO las mujeres que tienen más de 2 hijos o hijas son significativamente menos probables de trabajar por trabajo, también trabajan menos semanas, menos horas por semana y tienen menor ingreso relacionado al trabajo y el ingreso para toda la familia disminuye, bajo el esquema de MCO se combinan dos elementos el primero es el efecto del tamaño de la familia y el segundo es que todas las variables omitidas que están presentes en el tamaño de la familia, por tanto buscaremos una variale exógena en foma de un instrumento $Z$ para identificar la variación y aislarla, permitiéndonos encontrar un etimador causal plausible.

Así, esperaríamos que las variables omitidas que conllevan a que las familias tengan muchos hijas o hijos también reducen el esfuerzo en el trabajo por lo que estas dos cosas estarán positivamente correlacionadas lo cual implica que estaríamos obteniendo estimadores que son más grandes en magnitud que lo que realmente son, que es lo que vemos en la columna 2, cuando usamos 2SLS con los mismos instrumentos (que tus dos primeros hijos tengan el mismo género o no) esperaríamos que aislen la variación resultando en una variación menor de -.176 a -.120 para el trabajo por ingreso, de -8.97 a -5.66 en las semanas trabajadas, de -6.66 a -4.59 de horas por semana también será menor para el ingreso laboral y el ingreso familiar.

Así, para este caso podemos ver que al momento de estimar MCO es posible obtener estimadores sesgado dado las variables omitidas, por lo que si usamos variables instrumentales para identificar como si fuera una variación aleatoria en el tamaño de la familia y usaron eso para estimar los efectos que vemos están atenuados. 

```{r, out.width="60%"}
knitr::include_graphics("2SLS_gr1.png")
```


Así a forma de resumen podemos resumir los siguientes supuestos importantes para applicar 2SLS: 

1. E(u_{i}|W_{i1},...,W_{ri}) = 0 (Todos los otros controles son exógenos, i.e $W_{ri}$ van a ser exógenos a $u$)
2. (X_{1},...,X_{ki}, W_{ki}, W_{1},...,W_{ri}, Z_{1i},...,Z_{mi}, Y_{i}) son independientes e idénticamente distribuidas.
3. Curtosis finita para todo $X, W, Z$ y $Y$. 
4. $(\hat{X_{1i}*},...,\hat{X_{ki}}*, W_{1i},..,W_{ri},1)$ no son perfectamente multicolineales, donde $\hat{X_{ji}}$ son las predicciones de la población de $X_{j}$ sobre todas las $W_{j}$ y $Z_{J}$.
5. $Cov(Z_{1i}, u_{i}) = ... = cov(Z_{mi}, u_{i}) = 0$

Donde el supuesto 4. es la condición multivariada para la relevancia de un instrumento, los supuestos 1-5 hacen que $\hat{\beta^{2SLS}}$ sea consistente (converge al estimador verdadero) pero no insesgado y notemos que la $R^2$, $F-stat$ no son válidos. 

Así, recordemos que tenemos dos condiciones para los instrumentos: 

1. Exogeneidad: $Cov(Z,u)=0$
2. Relevancia: $Cov(Z,X)=0$


La exogeneidad generalmente no se cumplirá generalmente debido a que $Z$ está relacionada con otro factores que influencían $Y$. o bien $Z$ tiene un efecto directo sbre $Y$, así podemos ver que: 

* La exogeneidad se relaviona con $u$ - no es testeable. 
* Para el ejemplo que hemos visto cabe preguntarnos, ¿el género de un hijo o hija es completamente aleatorio? Mejor usemos loterías o RCT 


También notemos que la relevancia puede fallar si el instrumento es muy débil, lo cual sí puede ser testeable usando un F-test sobre los instrumentos en la first stage, solo tenemos que asegurarnos que $F$ es grande (tegla del meñique >10), también si el instrumeto es deil tendremos inconsistencia, y un sesgo incluso para muestras grandes y también afecta la normalidad, dificultando la forma de hacer inferencia. 

Por tanto, es importante tener preseente que las estrategias de variables instrumentales solo usan la variación relacionada con el instrumento, por lo que si todas las unidades tuviesen el mismo efecto del tratamiento, entonces $\beta_{i} = \beta \forall i$ no tendríamos problema, perosi tenemos efectos heterogéneos del tratamiento, tal que los individuos/unidades tienen $\beta_{i}$ distintas, entonces las variables instrumentales tienen una intepreteación específica. 

Así $\beta^{2SLS}$ será el Efecto del Tratamiento Promedio Local (LATE - Local Average Treatement Effect) dado que $\beta^{2SLS} = E(\beta_{i})$ para cada $i$ que es afectada por el instrumneto, la intuición de esto es que dado que la variable instrumental solo usa la variación en $X$ relacionado con $Z$, es decir estamos usando una submuestra de los datos para estimar la relación entre $X$ y $Y$, si esa submuestra no es representativa de la relación de $X$ con $Y$ solo obtendremos información sobre la relación en esa submuestra.

Podemos pensar en lo anterior recordando el instrumento para tener hijos o hijas del mismo género, ¿es el tamaño de la familia afectado por el instrumento por los primeros hijos/hijas del mismo género siempre? no realmente, sobre todo para familias que no tienen hijos/hijas o que tienen uno solo. 

Un buen instrumento tiene relevancia y también cunple con la restricción de exclusión; $\hat{\beta^{IV}} = \hat{Cov}(Y,Z)/ \hat{Cov}(X,Z))$ donde tenemos una $X$ y una $Z$ y usaremos 2SLS, sabemos que las IV pueden desiminuir su precisión dad o que solo usan una prporción de la variación en $X$, la restriccón de exclusión no se puede testear y generalmente fallará, por lo que tenemos que ser cuidadosos con esta metodología. 

A manera de resumen, podemos decir que las estrategías IV pueden solucionar el Sesgo por Variable Omitida y otros problemas que hacen que $E(u|X) \neq 0$, la forma general de su implementación es a través de 2SLS, un instrumento requería exogeneidad $Cov(Z,u) =0$ lo cual no se puede testear y también relevancia $Cov(Z,x \neq 0$ lo cual se puede testear, un instrumento débil es peligoroso dado que puede disminuir la precisión de nuestra estimación, además de ser sesgado, podremos usar el (estadístico) test $F$ sobre los instrumentos en la primera etapa, pero tenemos que asegurarnos que $F$ es suficientemente grande (generalmente >10) para omitir un problema de instrumentos débiles. 
